...
class TransformVisualization: NSObject, VirtualContentController {
    ...
    static var attentionSpan = 0.0
    static var cTime = 0.0
    ...
    func renderer(_ renderer: SCNSceneRenderer, nodeFor anchor: ARAnchor) -> SCNNode? {
        ...
        contentNode?.simdPivot = float4x4(diagonal: float4(0, 0, 0, 1))
    ...
    func renderer(_ renderer: SCNSceneRenderer, didUpdate node: SCNNode, for anchor: ARAnchor) {
        ...
        // new code
        TransformVisualization.attentionSpan = Double(abs(leftEyeNode.orientation.y) + abs(rightEyeNode.orientation.y))
        
        TransformVisualization.cTime = Double(CFAbsoluteTimeGetCurrent())
...
//         Scale down the coordinate axis visualizations for eyes.
        rightEyeNode.simdPivot = float4x4(diagonal: float4(0, 0, 0, 1))
        leftEyeNode.simdPivot = float4x4(diagonal: float4(0, 0, 0, 1))
