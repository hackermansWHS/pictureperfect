...
//  theyCallMeSIGABRT
//
//  Created by Kevin Pradjinata on 1/16/21.
//
...
class GraphViewController: UIViewController, CALayerDelegate, CPTPlotDataSource {
    ...
    @IBOutlet var postAnal: UITextView!
    ...
    func showPost(){
        postAnal.textColor = UIColor.white
        var txt4 = "Nothing printed..."
        if let totTime = totTime{
            let txt = "Time Elasped: \(String(format:"%.2f", elaTime)) seconds"
            if let wpm = wpm{
                let txt2 = "\nActual WPM: \(String(format:"%.2f", Double(wpm.last!))) words spoken per minute"
                if let homo = hOmO{
                    let txt3 = "\nYour Recorded Transcript: \(homo)"
                    txt4 = txt + txt2 + txt3
                }
            }
        }
...
//        let point = Double.random(in: 75...85)
        ...
        if self.currentIndex == wpm!.count-1{
            timer?.invalidate()
            print(showPost())
        }
//        xValue.text = #"X: \#(String(format:"%.2f",Double(self.plotData.last!)))"#
...
//        plot?.insertData(at: UInt(self.plotData.count-1), numberOfRecords: 1)
    ...
    func configureGraphAxis(){
        ...
        if let x = axisSet.xAxis {
            x.majorIntervalLength   = NSNumber(value: Double(wpm!.count/5))
